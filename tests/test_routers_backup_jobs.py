...
async def test_create_backup_job_happy_path(client, session):
    server, disk = await _server_and_disk(session)
    resp = await client.post("/api/backup-jobs", json=_job_payload(server.id, disk.id))
...
async def test_create_backup_job_disk_server_mismatch_is_422(client, session):
...
async def test_create_backup_job_disk_matching_server_still_succeeds(client, session):
...
async def test_create_backup_job_missing_server_is_404(client, session):
    _server, disk = await _server_and_disk(session)
    resp = await client.post("/api/backup-jobs", json=_job_payload(999999, disk.id))
    assert resp.status_code == 404


async def test_create_backup_job_missing_disk_is_404(client, session):
    server, _disk = await _server_and_disk(session)
    resp = await client.post("/api/backup-jobs", json=_job_payload(server.id, 999999))
    assert resp.status_code == 404


async def test_create_backup_job_on_deleted_server_is_409(client, session):
...
async def test_create_backup_job_with_deleted_sql_instance_is_409(client, session):
    server, disk = await _server_and_disk(session)
...
async def test_create_backup_job_sql_instance_without_verification_method_is_422(client, session):
    server, disk = await _server_and_disk(session)
...
async def test_get_backup_job_404(client):
    resp = await client.get("/api/backup-jobs/999999")
    assert resp.status_code == 404


async def test_update_backup_job_happy_path(client, session):
    server, disk = await _server_and_disk(session)
...
async def test_update_backup_job_sql_instance_without_verification_method_is_409(client, session):
    server, disk = await _server_and_disk(session)
    job = build_backup_job(server.id, disk.id)
    session.add(job)
    await session.commit()

    resp = await client.patch(f"/api/backup-jobs/{job.id}", json={"sql_instance_id": 1})
    assert resp.status_code == 409


async def test_update_backup_job_404(client):
    resp = await client.patch("/api/backup-jobs/999999", json={"name": "x"})
    assert resp.status_code == 404


async def test_delete_backup_job_with_active_run_is_409(client, session):
    server, disk = await _server_and_disk(session)
    job = build_backup_job(server.id, disk.id)
    session.add(job)
    await session.commit()
...
async def test_delete_backup_job_happy_path(client, session):
    server, disk = await _server_and_disk(session)
    job = build_backup_job(server.id, disk.id)
    session.add(job)
    await session.commit()
    ...
    get_resp = await client.get(f"/api/backup-jobs/{job.id}")
    assert get_resp.status_code == 404


async def test_delete_backup_job_404(client):
    resp = await client.delete("/api/backup-jobs/999999")
    assert resp.status_code == 404


async def test_list_backup_jobs_filters(client, session):
    server, disk = await _server_and_disk(session)
    job = build_backup_job(server.id, disk.id, is_enabled=False)
    session.add(job)
    await session.commit()

    resp = await client.get("/api/backup-jobs", params={"is_enabled": False})
    ids = [j["id"] for j in resp.json()["items"]]
    assert job.id in ids

    resp2 = await client.get("/api/backup-jobs", params={"is_enabled": True})
