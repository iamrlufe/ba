...
const jobFormSchema = z
  .object({
    ...
    source_path: z.string().min(1, "Required").max(500),
    backup_type: z.enum(["FULL", "DIFFERENTIAL", "TRANSACTION_LOG", "CUSTOM"]),
    schedule_cron: z.string().min(1, "Required").max(120),
...
export function JobFormPage({ mode }: { mode: "create" | "edit" }) {
  ...
  const updateMutation = useMutation({
    mutationFn: (values: JobFormValues) => {
      ...
      if (dirty.source_path) patch.source_path = values.source_path;
      if (dirty.backup_type) patch.backup_type = values.backup_type;
      if (dirty.schedule_cron) patch.schedule_cron = values.schedule_cron;
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select value={field.value} onValueChange={field.onChange} disabled={!serverId || mode === "edit"}>
                      <FormControl>
                        <SelectTrigger>
