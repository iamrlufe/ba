...
- ✅ Deployment-инфраструктура (`Dockerfile`, `docker-compose.yml`, `docker-entrypoint.sh`, `.dockerignore`,
  ...
  Uvicorn в `docker-entrypoint.sh` запускается с `--timeout-keep-alive 90` (добавлено поверх
  дефолта в 5s, который был короче polling-cadence агента — scheduler tick 15s / job-poll 30s /
  heartbeat 60s — и приводил к client-side SocketException 10054 + server-side teardown-race
  RuntimeError). НЕ проверено вручную с живым агентом и реальными idle-паузами >5s — покрыто
  только статическим тестом на наличие флага (`tests/test_docker_entrypoint.py`), который не
  запускает реальный uvicorn-процесс и не проверяет фактическое keep-alive-поведение. Требуется
  ручная проверка на реальном трафике агента при следующей возможности — тот же принцип честности,
  что и выше по этому пункту.
